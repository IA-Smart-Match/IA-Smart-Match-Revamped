...
| "Speaker" (role `speaker`) | Speaker Portal (B26) | A roster contact who accepted a portal invitation and signs in to see their own invitations, engagements, availability and contact preferences | A `speaker_profile` row by itself (most never sign in), and never a wider `volunteer` grant | `membership.role = 'speaker'`; `/v1/me/*`; `SPEAKER_PORTAL` (off by default) |
| binding (`account_user_id`) | Speaker Portal (B26 T6b-1, T6b-5) | Which login a `speaker_profile` is bound to, set by activation. `binding_mode` records how: `new_login` (the contact's own new login) or `existing_login` (an Event Host login proved with its password) | A merge of two people, or a copy of the Host's data into the profile | `speaker_profile.account_user_id` (`schema.py:2341`); `speaker_portal_invitation.binding_mode` (`schema.py:2605`) |
| "one login, two roles" | Speaker Portal (B26 T6b-5) | One Event Host login that also holds `speaker`, after an existing-login activation. Each role grants only its own routes; neither widens the other. Suspending the login suspends both | A staff or student login with a Speaker role — existing-login mode binds Event Host logins only | `tests/authz/test_policy_matrix.py::HOST_AND_SPEAKER_PERMITS` |
