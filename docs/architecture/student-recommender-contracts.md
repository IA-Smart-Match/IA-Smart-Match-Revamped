...
**Status:** Proposed with [ADR-0024](decisions/ADR-0024-staged-student-event-recommender.md); no route, module, or table described here exists yet.
...
`exclude_event_ids` is the whole of session adaptation (ADR-0024 D6). It is
never stored; it is folded into `inputs_hash` so two identical calls agree.

`wildcard_count` is the wildcard stream's continuation knob (ADR-0024 D7,
...
STUDENT_FEED_MAX_PER_PRIMARY_TAG: Final[int] = (
    3  # soft preference: items past this per-tag count are deferred, then fill open slots (ADR-0024 D7)
)
...
**Ranker fallback rule (ADR-0024 D4):** the router constructs the ranker through
