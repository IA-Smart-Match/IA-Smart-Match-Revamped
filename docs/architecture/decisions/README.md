...
| [ADR-0019](ADR-0019-declared-table-ownership.md) | Table ownership is declared as data before any schema split | Proposed | 8 September 2026 | Every one of the 43 tables names one owning bounded context and one writing service in `smartmatch_persistence/ownership.py`, checked both ways against `schema.py` by a test. Resolves match-run ownership explicitly — the API writes the request (the job and outbox rows, through `submit_command`); the worker writes the result (the `match_run` snapshot) — and records that ADR-0013's single-writer premise for `attendance_record` was unasserted. **Does not decide the `schema.py` split**, which is permitted only along declared ownership lines and only after this map is green. | — | — | — |
