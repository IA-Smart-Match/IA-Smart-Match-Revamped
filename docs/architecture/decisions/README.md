...
| [ADR-0025](ADR-0025-class-exercise-scope-shares-the-matching-mechanism.md) | The class exercise is a second product scope that shares the matching mechanism | Accepted | 16 September 2026 | **Accepted — adds a scope, re-decides nothing.** The Spring 2027 class exercise runs as `ProductScope.CLASS_EXERCISE`: its own `exercise_` tables keyed by dataset and team number with no tenancy and no principal, its own routers registered only in that scope so the CBA authenticated routers are unreachable rather than bypassed, and one shared domain mechanism. Ann Wang's four factors (same major; said interested; career goal fits; attended similar) are implemented once in `student_factors/` and composed by two `FactorRegistry` values: `EXERCISE_REGISTRY` (approved by the requirements document) and ADR-0024's `STUDENT_REGISTRY` (still proposed, still gated on OQ-SE-01). Two rankers over the same functions in opposite directions; an exercise-only tie-break (more on file, seniors first, then Ann's `tiebreak_order`, or a dataset-seeded fixed order for a file without one); hidden true interests and hidden true career goal never serialized, read only by the deterministic per-team simulated-results rule and by the per-team refresh that copies them onto a card; no numeric score on any screen. Rejects a standalone scorer, a per-route auth bypass, a separate repository, and any identifier containing "demo". | 25 September 2026 — the tie-break's last step is Ann's `tiebreak_order`; D6 withholds two columns and the refresh reads them (see [`class-exercise-decisions-2026-09-25.md`](../../decisions/class-exercise-decisions-2026-09-25.md) D3, D6) | — | — |
| [ADR-0026](ADR-0026-student-program-and-student-centric-classroom.md) | The student program, and the student-centric classroom | Proposed | 17 September 2026 | **Proposed — records a priority and a discipline, licenses nothing.** The student is the subject of the next phase, per the 12 September 2026 stakeholder direction (student-to-event matching plus notifications that drive registrations; speaker matching, AI features, and mobile deferred; rewards a parallel second track). The canonical eight-slice engagement program is the platform shape, and each slice names the OQ-SE/OQ-SC row it waits on: registration QR (OQ-SE-03), host drafts and review (OQ-SE-09/10/13), the vertical flow, private media (OQ-SC-05, OQ-SE-11/12), announcements and portal parity (OQ-SE-14/15), supporting ranking (OQ-SC-02, OQ-SE-01/02), outbound digest (OQ-SC-03, OQ-SC-10, OQ-SE-16/17), and registered engagement metrics (OQ-SE-04..08, with W4 still STOPPED). Gates fail closed: an open gate means the slice stops at a plan, not code. ADR-0025's `ProductScope.CLASS_EXERCISE` is the student-centric classroom where the shared mechanism is proved over fictional rows — one `student_factors/` module, two registries, two processes — and nothing it produces closes a register row or moves real data. Both stakeholder tracks proceed. | — | — | — |
| [ADR-0027](ADR-0027-registry-3-engagement-load.md) | Registry 3.0.0: engagement load as a Stage A cap and a Stage B multiplier | Proposed | 23 September 2026 | **Proposed — declared, not current.** Registry `3.0.0-approved-b26-eli` adds `engagement_load` (PENALTY, weight 0, in no model) and the Q7 = A band table with its ownership record. Full is removed before the solve (`load_full`); Moderate and Heavy multiply the unrounded composite by 0.90 / 0.70; Light and Unknown by 1.0. For 3.x, `registry_hash` covers the weights and the canonical band table; 1.1.1 and 2.0.0 hashes are unchanged (amends ADR-0016's `registry_hash` rule for 3.x only). `REGISTRY_VERSION` stays 2.0.0; the one-line flip of `CURRENT_CBA_REGISTRY` waits on approval. | — | — | — |
