...
| [ADR-0016](ADR-0016-cba-scoring-policy.md) | CBA scoring policy: neutral Topic, proximity bands, and virtual redistribution | Proposed | Drafted 5 September 2026 — proposal only, no decision date | **Proposal only — decides nothing yet.** Puts nine numbered proposals to OQ-CBA-004's named owner: a third evidence state (`policy_neutral`) separating the customer's §9 neutral Topic score from a genuine `unknown`; the neutral value as a versioned policy constant with its own provenance; the exact 0–25 / 25–75 / 75+ proximity sub-scores with lower-inclusive, upper-exclusive boundary ownership; a missing address as `unknown` rather than the far band; virtual events as a pinned scoring mode with a stated redistribution formula; and the serialization, UI labels, `REGISTRY_VERSION` pin policy, and golden cases each of those implies. Refines ADR-0011 without amending it. | — | — | — |
