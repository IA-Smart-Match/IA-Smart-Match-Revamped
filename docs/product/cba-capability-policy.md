...
| `speaker_portal` | **off** | **off** | **off** | B26 T6b-1: Speaker accounts — a Speaker Connector invites a contact, the contact activates a `speaker` login from a one-time `/s/{token}` link. Built and **staged off in every scope**. Turning it on is a reviewed edit to the policy rows, never an env var, seed or compose file. **Turn-on rule (C5, owner-confirmed):** T6b-5 merged **and** parent plan §10 rows 1 (Ann/Pia/Lisa), 2 (named privacy owner) and 4 (pilot hostname) cleared. Requires `authenticated_login`, `consented_outreach` and `speaker_contact_management` in the same scope (asserted at import). |
