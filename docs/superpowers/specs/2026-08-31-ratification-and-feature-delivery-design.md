...
| P9 Gate B | Session approver: Danny Tran (`dt110202@gmail.com`); formal gate still requires Dr. Wang plus a named privacy owner | Collect Public URL, contact name, and email/phone when available for IA West Coordinator follow-up. | **RECORDED — GATE INCOMPLETE** | Named privacy owner; purpose; minimization; retention; correction/deletion; viewer/exporter roles; per-field decisions; signatures. | Only validated **HTTPS public event-page URL** support. Contact data may not be collected, persisted, or quarantined. |
...
| P8 opportunities | Session approver: Danny Tran (`dt110202@gmail.com`); formal gate still requires the product owner | The programmatic opportunity set includes hackathons, datathons, competitions, guest lecturer events, and school events; unknown categories are intended for IA West Coordinator review. | **RECORDED — GATE INCOMPLETE** | Whether the examples are exhaustive; canonical eligibility/count definition; owning evidence; T-28 authority; P6 persistence. | Committed category-shape fixtures only. No durable assignment, approval/rejection action, metric, or publication. |
...
| P6 Stage 0 scope | Session approver: Danny Tran (`dt110202@gmail.com`) | iCal and JSON-LD parser work is in scope. | **RECORDED — GATE INCOMPLETE** | The unsigned R3/P6 gate and T-27–T-29 remain open. | Parser registration/public API, committed synthetic fixtures, and pure candidate mapping with no persistence, network, or model call. |
...
| V2 | P9 pilot columns | Validated HTTPS Public URL only; contacts and `board_role` behavior remain blocked. |
| V3 | P6 event discovery | Registered parsers, committed fixtures, and pure candidate mapping only. |
...
Only `Public URL` may proceed now, and only as a validated **HTTPS public
event-page URL**. Validation rejects non-HTTPS schemes, embedded credentials,
private subscription tokens, loopback/private/link-local destinations, and
values that are not public event pages. The final network-egress controls still
apply before any future fetch; this slice validates stored/imported values and
does not fetch them.
...
Tests in this slice cover only HTTPS Public URL validation and prove that the
URL path neither performs a network call nor opens a contact-data path. The
existing P9 plan must be amended or superseded before any replacement card
executes.

## 7. V3 — P6 offline event ingestion

The unsigned P6/R3 stop-gate is not passed. Authorized early work is limited to
a provider-free, side-effect-free parser boundary:

```text
committed synthetic fixture
  -> registered iCal or JSON-LD parser public API
  -> ParsedSourceEvent
  -> pure candidate mapping
```

The parser registry and public API accept only the bounded fixture shapes
committed with the task. Pure candidate mapping performs no persistence,
network access, model dispatch, review assignment, publication, or UI action.
No provider interface or fake provider adapter lands, and no arbitrary
...
quantified input; it can claim only the named formats and bounded committed
fixtures its tests exercise. Malformed or unsupported fixture cases fail
...
authorized yet. The design contract keeps raw rows restricted and fail-closed;
the current intentionally ungated route is a known implementation gap, not an
approved policy and not a pattern new work may copy. When the gate closes, raw
row refusal must use the standard error envelope rather than an empty row list,
and aggregate/drill-down equality remains required for authorized callers.
...
may prove the shapes of known and unknown categories without waiting for P1 or
P9.

The later session direction intends unknown-category events to go to the IA
West Coordinator for review. Executable assignment and approve/reject actions
...
(`dt110202@gmail.com`) was named in the handoff, the IA West Coordinator is the
