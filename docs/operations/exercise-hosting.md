...
2. **Confirm the migration head is `0039_speaker_portal`.**
   ```bash
   grep -L 'down_revision = "0039_speaker_portal"' /dev/null; \
   grep -rl 'down_revision = "0039_speaker_portal"' db/migrations/versions/*.py
   ```
   Pass: the second command prints **nothing** — no later revision points back
   at `0039_speaker_portal`, so it is the head
   (`db/migrations/versions/0039_speaker_portal.py:39` sets its own
   `down_revision = "0038_speaker_availability"`). Fail: a revision is printed —
   the head has moved past `0039`; re-derive this step against the new file
