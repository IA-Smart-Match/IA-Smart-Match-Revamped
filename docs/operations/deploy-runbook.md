...
| Apply infrastructure | F5 landed the modules — Cloud Run, Cloud SQL 16, Cloud Tasks, Cloud Scheduler, Secret Manager placeholders, buckets — under `infra/terraform/modules/`, composed per environment by `modules/platform`. **None of it has been applied and `terraform apply` has never been run.** There is no root module, no provider block and no state backend, and `modules/platform` requires four inputs that name things nobody can supply today (both container images, the worker URL, the scheduler audience), so a plan cannot be produced either. `make infra-check` fails if any of that changes | The program owner setting `ALLOW_CLOUD_DEPLOY=true`, plus a registry and the S-001 identities |
