...
| **D2** | ELI formula parameters (decay half-life, window, caps) | The parameters implemented today stand as the tentative values. The open sub-question — whether committed future engagements count toward load — stays open; current behaviour refuses them explicitly rather than dropping them silently. **B26, 2026-09-22:** DECIDED 2026-09-22 (owner): option B — build self-service availability; plan at [`docs/plans/2026-09-22-b26-self-service-availability-plan.md`](../plans/2026-09-22-b26-self-service-availability-plan.md). That decision does **not** answer this sub-question — it chooses to collect availability, not whether committed future engagements count toward load — so D2 stays **OPEN**, and the plan's ELI track (T8) waits on it. | Confirmation or replacement of the parameters; **and** an answer to the committed-vs-completed sub-question (plan owner question Q1), which ELI needs before any availability data can feed it. |
