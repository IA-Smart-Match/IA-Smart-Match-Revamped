...
> **Partially superseded — 27 August 2026.** This document locates the code and
> YAML corrections on `claude/pr1-blockers-todos-er5heu`; that branch merged as
> **PR #3** (`9865bf3`), so they are on `main` now. The F9 manifest counts
> (MM-003, MM-004, MM-005) were independently re-measured and confirmed — see
> §5 of `docs/plans/pr3-verification-evidence.md`. What remains open is the
> `ported_unverified` status itself, which this document is still the plan for.
