...
| J11 | A 409 from re-drive or abandon poisons its idempotency key | J4 | **Found by the Wave C audit, in Wave B code; verified by reading the path.** `redrive_job` commits after `_redrive.redrive` raises `RedriveConflictError` or `InvalidTransitionError` (`routers/redrive.py`, the 409 path). The commit is deliberate and its comment is right about *why* — an uncommitted rate-limit increment would let a caller hammer an un-re-drivable job for free (S-008) — but it also commits the reservation `_reserve` inserted moments earlier. A retry with the same `Idempotency-Key` then takes the replay branch and answers **202 with `replayed: true`** for a re-drive that was refused and never happened. `abandon_job` has the identical shape, returning `200 {"status": "abandoned"}` while the job is still `failed_provider`, with no audit record and no state change. `commands.py` never commits a reservation it just created, and `IdempotencyRepository.reserve`'s docstring states the rule ("must run in the same transaction as the job and outbox inserts, so a reservation is never left behind by a rolled-back command"). The fix has to keep the quota and drop the reservation — they are currently the same commit. |
