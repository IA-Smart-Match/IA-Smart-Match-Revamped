...
> **Superseded for both defects — 27 August 2026.** CP-GRANT and CP-A5 shipped in
> **PR #3** (`9865bf3`): `services/api/smartmatch_api/job_authz.py` is the single
> authorizer behind job status, events, re-drive and abandon, and migrations
> `0006`/`0007` are on `main`.
>
> **One design point below was deliberately not taken.** §2(c) specifies
> `job.owning_unit_id UUID NULL` with a persisting NULL fallback ("Command types
> with no unit leave NULL. Modelled, not a gap."). What shipped adds the column
> nullable, backfills it, and applies `SET NOT NULL` **in the same transaction**,
> because there is no production data and no rolling deploy to justify an
> expand/contract split — and because, in `0006`'s own words, "a nullable
> authorization input is a fail-open shape waiting to be written". Read §2 as the
> plan of record for *why* the column exists, not as a specification of its shape.
