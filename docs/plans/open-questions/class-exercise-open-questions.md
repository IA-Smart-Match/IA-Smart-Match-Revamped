...
| OQ-CE-13 | When a refresh copies a card onto a profile, what career goal does the copied card carry — the one already on the base row, or nothing? | Ann | Owner ruling 2026-09-21 (Danny): **copied cards carry the base `career_goal`; `NULL` only when the base row has none.** A safe default, not Ann's answer. **The ruling is recorded here; it is not yet in the code.** On `main` today the refresh leaves `card_career_goal` `NULL` and the goal is resolved through the base row instead (PR #190, owner question 4) — which is why the question arose. A switchable policy marked `PLACEHOLDER (OQ-CE-13)` is follow-up work on `feat/exercise-copied-card-goal`, not merged; `grep -rn "OQ-CE-13" services/ python/` is empty on `main`. | Refresh | OPEN — Ann may replace the default; the default is not yet implemented |
