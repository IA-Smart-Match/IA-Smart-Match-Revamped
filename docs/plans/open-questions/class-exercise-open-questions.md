...
| OQ-CE-13 | When a refresh copies a card onto a profile, what career goal does the copied card carry — the one already on the base row, or nothing? | Ann | Owner ruling 2026-09-21 (Danny): **copied cards carry the base `career_goal`; `NULL` only when the base row has none.** A safe default, not Ann's answer. Marked `PLACEHOLDER (OQ-CE-13)` in the refresh, and switchable in one place. Raised as owner question 4 on PR #190, where the refresh left `card_career_goal` `NULL` and resolved the goal through the base row instead. | Refresh | OPEN — Ann may replace the default |
