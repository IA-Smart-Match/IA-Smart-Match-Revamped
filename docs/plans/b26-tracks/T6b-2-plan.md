...
| `services/api/smartmatch_api/routers/cba_invitations.py` (Connector view, C2) | `SpeakerResponseView.channel` description (`:292-299`) gains `'speaker_portal'`: the Speaker answered while signed in. **Code change (orchestrator ruling 2026-09-23):** `_outcome_view` (`:541-573`) today passes `response_recorded_by_user_id` straight through (`:572`). Under C2 = (b) the database also sets it for `speaker_portal`, so the pass-through would show Connectors the Speaker's login id, which after T6b-5 may be a merged Event Host account. It becomes `recorded_by_user_id=row.response_recorded_by_user_id if row.response_channel == "connector_recorded" else None`. The database keeps the attribution; only the Connector view hides it. `recorded_by_user_id` description (`:301-303`) becomes: "The coordinator who entered it. Null for a Speaker's own answer, by link (`speaker_link`) or signed in (`speaker_portal`); a signed-in answer's login is recorded but not shown here." |
| `apps/web/legacy-frontend/src/lib/api.ts` (Connector types, C2) | `SpeakerInvitationResponse.channel` JSDoc (`:3090-3095`) and `recorded_by_user_id` (`:3098`) say the same: set only for `connector_recorded`. |
...
31. `TestSpeakerPortalAnswer::test_a_portal_answer_hides_the_speakers_login_from_the_connector` (a bound Speaker answers through `/v1/me`; the batch read shows `channel: "speaker_portal"` and `recorded_by_user_id: null`; the stored row still holds the login id; the login id appears nowhere in the raw response body)
32. `TestSpeakerPortalAnswer::test_a_link_answer_still_has_no_recorded_by` (`speaker_link` → `recorded_by_user_id: null`)
33. `TestSpeakerPortalAnswer::test_a_connector_recorded_answer_still_shows_the_connector` (`connector_recorded` → `recorded_by_user_id` = the coordinator's id, unchanged behaviour)
...
7. `feat: Connector view hides the portal answer's actor; SPEAKER_PORTAL policy row` — the `_outcome_view` gate, the two `cba_invitations.py` descriptions, the `api.ts` JSDoc, the policy row, then `make openapi VENV=$VENV` and the regenerated `contracts/openapi/smartmatch.json` (description changes only). Contract tests 31–33 go red in milestone 3 and green here.
