...
2. `invitations_paused_until is not None and invitations_paused_until >= as_of` → `BLACKED_OUT / PAUSED`. Pause does not read the event date (§3.1 note). See §6 C1.
3. `event_dates is None` → `UNKNOWN / EVENT_UNRESOLVED`.
...
| `CAPACITY_MIN_EXCLUSIVE` / `CAPACITY_MAX` | 0 / 720 | `0 < capacity <= 720`, at most 1 decimal place (§6 C3) |
...
7. `test_pause_wins_over_unresolved_event` (§6 C1)
...
14. `test_empty_event_dates_raises_value_error`
...
34. `test_capacity_720_valid` / `test_capacity_720_1_invalid` / `test_capacity_two_decimals_invalid`
...
Types: 38. `test_types_are_frozen`. In `tests/unit/test_eligibility.py`: `test_evidence_reason_defaults_to_none`, `test_evidence_rejects_mismatched_reason`, `test_positional_evidence_still_constructs`.
...
|---|---|---|---|
| C1 | §5.1 table lists "Event `unresolved`" above the pause row. The pause compares with `as_of`, not the event date (§3.1), so a paused Speaker is not invitable for any event. | (a) table order: unresolved → `UNDETERMINED` even when paused; (b) pause first → `EXCLUDED`. | **(b)**. Moot in practice: ADR-0010 keeps unresolved events out of matching. Dispatcher may flip; test 7 pins it. |
| C2 | §4.1 lists window failures as "order, span or horizon" only; T2's `uq_speaker_availability_window_range` would reject a duplicate as a DB error. | (a) map duplicate to `window_invalid`; (b) new code. | **(a)**, no new API code. |
| C3 | `numeric(5,1)` silently rounds `24.05` to `24.1`; §4.1 only says "≤ 0 or > 720". | (a) reject > 1 decimal as `capacity_invalid`; (b) round. | **(a)**: no silent change to a stated number. |
