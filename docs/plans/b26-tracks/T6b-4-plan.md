...
**Next action:** wait for the §0 start gate, then run milestone 0 (the three merges).
...
Inputs (all final): T6b-1 plan `origin/feat/b26-t6b-1` @ `3e9a7ef4` (rev 3); T6b-2 plan `origin/feat/b26-t6b-2` @ `8805cdc3` (rev 2); T6b-3 plan `origin/feat/b26-t6b-3` @ `542866ca` (rev 2); T5 plan `origin/feat/b26-t5` @ `f0c060e5`; T3 plan `origin/feat/b26-t3` §5 (types).
...
1. `git merge origin/feat/b26-t6b-2`. It already carries T8a → T6b-1 → T2 → T1 and T3.
2. `git merge origin/feat/b26-t6b-3`. Common base is T6b-1. Expected textual conflicts, all resolved as a union:
   - `services/api/smartmatch_api/main.py` (`CAPABILITY_SCOPED_ROUTERS` rows, router imports)
   - `tests/authz/test_policy_matrix.py`: both tracks replace T6b-1's `test_a_speaker_membership_reaches_no_operation`. Keep **one** test and one `SPEAKER_SELF_OPERATIONS` holding all 8 keys (T6b-2's 5 + T6b-3's 3).
   - `tests/authz/test_route_roles.py`: both add `_SPEAKER_SELF = frozenset({"speaker"})`. Keep one literal and both sets of rows.
   - `src/lib/api.ts` (T6b-3's `speaker_choice` fields next to T6b-2's adapters)
   - `contracts/openapi/smartmatch.json`: never hand-merge. Take either side, then `make openapi VENV=$VENV`.
3. `git merge origin/feat/b26-t5`. Common base is T3. Expected conflict: `CoordinatorSpeakerContacts.tsx` `ContactRow` (T5's "Availability" disclosure next to T6b-1's `SpeakerPortalInvite`); keep both elements.
4. The PR targets `main`. Body line 1: **"Merge #210, #212, then the T6b-1, T8a, T3, T6b-2, T6b-3 and T5 PRs first."** After they land, merge `main` into `feat/b26-t6b-4` so the diff is T6b-4 only.
5. T6b-4 writes no migration and no Python source. Backend conflicts in step 2 are merge hygiene, not T6b-4 design: if they need more than a union, stop and escalate to the orchestrator.
...
| Path | Change |
|---|---|
| `src/lib/api.ts` | **3 adapters and their types** for `/v1/me/contact-channels` (§6.1), after T6b-2's `fetchMyEngagements`. If T6b-2 as built lacks `"speaker_invitation_response_conflict"` in `SpeakerSelfErrorCode`, add it (T6b-2 §2.3 returns it; its §6 union omits it). |
...
6. **T6b-5 slots.** Two JSX comments, nothing rendered:
   - sidebar footer, between the identity block and Sign out: `{/* SLOT(T6b-5): portal switcher, shown only with 2+ portals */}`;
...
└─ shell
   ...
   │  └─ footer: initials · name · unit path · "Speaker Portal" · SLOT(T6b-5) · Sign out
   ...
   └─ main → Outlet
      ...
      │  └─ section h2 "Answered"                 → InvitationRow (read-only)
...
|---|---|---|---|---|
| `useAnswerMyInvitation()` | `answerMyInvitation(invitationId, response)` | `return queryClient.invalidateQueries({ queryKey: [pk, "my-invitations"], exact: true })`. Returning the promise keeps `isPending` true until the re-read lands, so the buttons stay disabled until the row has moved. | `speaker_invitation_not_found`, `speaker_invitation_already_answered`, `speaker_invitation_response_conflict` → the same exact invalidation | `my-engagements` (an accept writes no pipeline row, T6b-2 §2.3), `my-availability`, `my-contact-channels` |
...
- **One mutation in flight per page.** While any answer (or channel choice) is pending, every answer (or choice) button on that page is disabled; the pressed one reads "Saving…".
...
| I2 | List | "Waiting for your answer" (`answerable`), then "Answered". Each row: title; "Date in the invitation: {date_text}" verbatim; the formatted `local_date` when present; "Sent {dispatched_at}" (not "delivered": T6b-2 §2.2). |
...
| I5 | Pending | All answer buttons disabled; the pressed one reads "Saving…". |
...
| E3 | List | `EngagementRow`: title; when (§6.3); state chip Confirmed / Attended / Cancelled (icon + word). Cancelled adds "Cancelled on {date}" and never who cancelled. `event: null` → "Event details are no longer available." |
...
| C5 | Pending | All choice buttons disabled; the pressed one reads "Saving…". |
...
| Go back | "Go back" | "Keep invitation to {title} unanswered" |
...
All are native `<button type="button">`. The confirm row is inline, not a modal (`RedemptionTicketRow.tsx:118-200`): opening it moves focus to the Confirm button; "Go back" / "Keep receiving" closes it and returns focus to the button that opened it.

Focus after a success: once the re-read lands, focus moves to the affected row's `h3` (`tabIndex={-1}`, found by its id). For an answer that is the row now under "Answered"; for a channel it is the same row, whose button has flipped. Focus is never dropped to `<body>`. After an error, focus stays on the pressed button, which stays mounted.
...
4. `Accept opens the confirm row and moves focus to Confirm accept; Go back returns focus to Accept`
5. `Confirm accept sends exactly one POST {"response":"accept"} to the encoded id`
6. `buttons stay disabled and read Saving… until the list re-read resolves` (deferred GET after the POST)
...
12. `accessible names carry the invitation title` · `truncated says 200`
...
5. `buttons stay disabled until the re-read resolves; success invalidates exactly [principal, "my-contact-channels"]`
...
5. `test_the_t6b5_and_t8d_slots_are_marked`: `SLOT(T6b-5)` twice in `SpeakerPortalLayout.tsx`; `SLOT(T8d)` once in `SpeakerOwnAvailability.tsx`.

Plus the `test_frontend_auth_contract.py` tuple edits (§1), and, unchanged and run one at a time: `test_frontend_paged_list_contract.py`, `test_frontend_no_fake_success_contract.py`, `test_frontend_zero_coercion_contract.py`, `test_frontend_portal_landing_contract.py`, T6b-2's `test_frontend_speaker_self_contract.py`, T3's `test_frontend_speaker_availability_contract.py`.
...
0. `chore: merge origin/feat/b26-t6b-2 into feat/b26-t6b-4`, `chore: merge origin/feat/b26-t6b-3 …`, `chore: merge origin/feat/b26-t5 …` (§0; `make openapi VENV=$VENV` if the JSON conflicted).
1. `test: speaker portal adapters, error map and formatters (red)` — §9.2 A, B, C; §9.3 items 1–2.
2. `feat: /v1/me/contact-channels adapters, speaker portal error map and formatters` — A, B, C green.
3. `test: speaker Home, Invitations and Engagements pages (red)` — D, E, F.
4. `feat: speaker Home, Invitations and Engagements with useSpeakerSelf hooks` — D, E, F green.
...
7. `test: speaker portal shell, routes and prefetch (red)` — I, J, K; §9.3 item 5; the `test_frontend_auth_contract.py` tuple edits.
...
9. `docs: DESIGN.md Speaker Portal shell` — plus the browser check at 360, 768, 1024 and 1440 with the capability flipped on locally (not committed), recorded in the PR.

Before each push: `$VENV/bin/ruff format` and `ruff check` on touched Python and this plan.

PR title: `feat: Speaker portal shell and pages (B26 T6b-4)`. Body: line 1 from §0; the test plan (11 Vitest files, tsc, the Python scans); the widths checked; ends with the Claude Code line; edited with `gh api -X PATCH …/pulls/N -F body=@file` (env rule 9).

## 11. Contradictions checked

| # | Found | Handling |
|---|---|---|
| X1 | T6b-2 §3.1 resolves the subject first (unbound → 404 for every role, then 403). T6b-3 §5.2 checks the `speaker` membership first (→ 403, then 404), in a separate `speaker_subject.py`, while T6b-2 §3.1 says T6b-3 reuses `_authorize_speaker_self`. | The UI handles both codes on every page (§5 common rule), so either order works here. Backend: OQ-4. |
| X2 | T6b-2 §6's `SpeakerSelfErrorCode` omits `speaker_invitation_response_conflict`, which §2.3 returns. | T6b-4 maps it by string (§7) and adds it to the union if absent (§1). |
...
| OQ-2 | Where exactly does T6b-5's switcher go? The parent says "header", but on `lg` and up the shells have no header, only the sidebar (DESIGN.md `:156`: no desktop top bar). | **Sidebar footer** (reachable at every width) **plus the mobile header**. Both slots are marked (§2.2); T6b-5 decides and builds. |
...
| OQ-4 | T6b-2 and T6b-3 disagree on subject-resolution order and on which module owns it (X1). | **T6b-3 reuses T6b-2's `_authorize_speaker_self`**, as T6b-2 §3.1 states: one authorizer, one order (404 first, then 403). Settle it on the T6b-3 branch before it merges; T6b-4 is unaffected either way. |
