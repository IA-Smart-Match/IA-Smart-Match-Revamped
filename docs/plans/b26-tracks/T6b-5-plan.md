...
| `ONE_IN_TENANT`, holder is not `S` | Owner ruling R-B (2026-09-24; PR #224 review ruling, not §1's R-B): only when every role in `R` is `volunteer` **and** the holder's active roles ⊆ {`speaker`, `volunteer`} (read under the locks above): `find_or_add_role(E, r)` for each role on the holder; **no** password change; no `S` account is created. Otherwise `SeedConflictError` naming the role and `<email_var>`, never the holder or its roles | stderr: "E already signs in as another login; roles … added to it; `<password_var>` was not applied" (Q4) |
