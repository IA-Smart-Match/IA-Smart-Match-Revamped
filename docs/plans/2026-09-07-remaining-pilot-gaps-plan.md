...
## 6. Sequencing, parallelism, and the summary

### 6.1 Classification at a glance

| Gap | Classification | What it reverses | Register action | Migration |
|---|---|---|---|---|
| 1 Host-scoped read | Reverses a recorded decision | OQ-CBA-014's safe default (`cba-phase-deferred.md:30`); `routers/speaker_requests.py:118-126` | Close OQ-CBA-014 (7 Sep 2026, Danny Tran); register OQ-CBA-065 | **`0033_event_filed_by`** |
| 2 Unit feedback aggregate | Decision-free | Nothing; a second Connector read under OQ-CBA-003's rule | Addendum under the OQ-CBA-003 decision | None |
| 3 Attendance route | Reverses a recorded decision | `attendance.py:21` "none may"; `routers/pipeline.py:61`, `routers/cba_handoff.py:46`; OQ-102's default (`pipeline-stage-writers-deferred.md:56-72`); exceeds synthetic-pilot authorization item 3 | Close OQ-102 in its own register; cross-reference row in the CBA register | None |
| 4 Funded reward item | Reverses a recorded decision | Worksheet status "do not seed listable catalog rows" (`rewards-catalog-worksheet.md:3`); `routers/rewards.py:105`; `generate_pilot_dataset.py:66-76` | Decision row in the CBA register; D6 records untouched | None |
| 5 `feedback.py` map | Decision-free | Nothing; `REGISTRY_VERSION` untouched | MM-005 `corrections` note in the manifest | None |

Only Gap 1 carries a migration, so the one-revision-per-PR rule and the migration head
never collide across these five: `0033` is the next and only revision, and it is on
Gap 1's branch.

### 6.2 Safest execution order

1. **Gap 5** first. Three files, no route, no register closure, no OpenAPI change. It
   touches nothing another gap touches and is the cheapest way to prove the pipeline
   (ruff, both scanners, unit tests) is green before the larger PRs.
2. **Gap 2** second. No migration; touches the feedback domain/persistence/router,
   `CoordinatorHome.tsx`, `api.ts`, the matrix, and the contract. It does not depend on
   Gap 3 for its route, and its e2e assertion is the suppressed case, which holds on
   today's appliance.
3. **Gap 1** third. The migration lands here; `make migrate-check` and the schema
   mirror test are the gate. Its frontend touches `VolunteerPortalLayout.tsx` and
   `routes.tsx`; rebase on the merged TRACK 15 work before the frontend milestone.
4. **Gap 3** fourth. It retires the prohibition, changes `record_attendance`'s return
   type (two callers), and rewrites e2e steps 24 and 25. Step 25's submission path also
   exercises Gap 2's unit route once both are merged.
5. **Gap 4** last. The repository writer and the seed tool are independent, but step
   15's request-and-decide branch needs a *measured* balance, which only exists after
   Gap 3's route has credited an attendance. Merge the tool early if convenient; the
   e2e milestone (4.4 item 4) waits for Gap 3.

### 6.3 What can run in parallel

Gaps 5, 2, 1 and 3 touch disjoint code paths and can be developed concurrently on four
branches. The files that *would* conflict, and how to handle them:

* `tests/authz/test_policy_matrix.py` — Gaps 1, 2 and 3 each append one `Operation`
  and one rectangle. Appends at the end of `OPERATIONS` and `MATRIX` merge cleanly if
  each PR adds its block *after* the last existing entry rather than next to a
  related one; the merger re-runs the file.
* `contracts/openapi/smartmatch.json` — regenerated by Gaps 1, 2 and 3. Never
  hand-merge it; the second PR to land re-runs `make openapi` after rebasing and
  commits the result.
* `tests/e2e/test_pilot_clickthrough.py` — Gaps 1 (step 26), 3 (steps 24, 25) and 4
  (step 15) edit different functions; a rebase is mechanical.
* `apps/web/legacy-frontend/src/lib/api.ts` — Gaps 1 and 2 each add one exported
  function; append-only.
* `docs/plans/open-questions/cba-phase-deferred.md` — Gaps 1, 2, 3 and 4 all write
  into the new `## Decision taken 2026-09-07` section. The first PR to land creates the
  section; the others add rows. The OQ-CBA-014 row rewrite (Gap 1) and the OQ-CBA-003
  addendum (Gap 2) are on different lines.

Gap 4's non-e2e milestones can run in parallel with all of the above; its e2e
milestone is the one sequential dependency in this plan.

### 6.4 What every PR must do before it opens

Run `python3 tools/scan_forbidden.py`, `python3 tools/scan_cba_terminology.py` and
`ruff format --check .`, and paste the output. Run the test files the section names,
plus `tests/unit/test_matching_fail_closed.py`, `tests/unit/test_checkin_wiring.py` and
`tests/authz/test_policy_matrix.py` regardless of which gap it is, because those three
are the files that fail when a route lands somewhere it was not meant to. State the
trade-off report in the body under the five headings each section ends with. Where a
recorded decision is reversed, the PR body links the register section that records
the reversal, so the owner reviews the decision and the code in one place.
