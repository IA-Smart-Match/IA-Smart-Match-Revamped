...
- **Backend mapping** is against `contracts/openapi/smartmatch.json` as it
  stands today (seven endpoints, measured) plus domain packages under
...
| H22 | `Calendar.tsx` / `StudentEvents.tsx:159–207` | Times/dates rendered without an IANA zone or precision. Student calendar is a month grid labeled “Google Calendar Style Mock”. | Event-local zone named; `date_only` as a date; agenda not a month grid (ADR-0010, engagement-model.md §5). |
...
| 1.2 W2 | Generate TS client from `contracts/openapi/smartmatch.json`; CI drift check. | 1.1 | Client matches the seven endpoints; no hand-written `/api/data/*`. |
