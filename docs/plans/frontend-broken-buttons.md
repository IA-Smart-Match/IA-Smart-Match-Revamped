...
| B09 ✅ *closed — a day opens its events* | `student/StudentEvents.tsx:356–573` `MonthCalendar`; day button `:481–512`, day panel `:522–570` | Prev/next month buttons (`:439–452`) work. A day with events is a `<button type="button">` named “Events on 18 September, 2 events” (`:488`) that opens a Radix dialog titled “Events on 18 September 2026”, listing that day’s events with the same `EventCard` the lists use — so **Register / Cancel registration** send the lists’ own `POST` / `DELETE …/student/events/{event_id}/registration` and the page re-runs both reads. Escape and Close return focus to the day (`:530`); one polite `role="status"` region (`:548`) announces what the re-read shows, a refusal stays the card’s `role="alert"`. Days with no events are plain cells, not buttons (the B41 pattern: nothing to open, no control). No filter state; both lists keep every event. | Time-ordered agenda of registered + open events (Fix #10). | Event read API: **shipped** (`/student/events`, `/student/agenda`). Registration: **shipped** (B06). No new endpoint. | **Closed by `fix/open-broken-buttons`** under the owner’s ruling on **OQ-CBA-020, option A (2026-09-22)**: a day opens a panel of its events with the lists’ Register/Cancel; no filtering. `StudentEvents.test.tsx` (9 tests: named day buttons, no control on empty days, panel POST/DELETE equal the lists’, live-region outcome, refusal shown, Escape/Close focus return, lists unfiltered); ordering still pinned by `tests/unit/test_student_events_layout_contract.py`. | — |
...
Still open and not deletable: B26 — DECIDED 2026-09-22: option B (build
self-service availability); planned on branch `docs/b26-self-service-availability-plan`.
Still open until built. B09 is closed (OQ-CBA-020 option A, 2026-09-22: a day opens
its events with the lists' Register/Cancel). B41/B42 are closed: both open the
shipped S1 drill-down.
