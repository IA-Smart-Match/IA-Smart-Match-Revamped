...
| B07 | `student/StudentEvents.tsx:128–132` **Add to Calendar** | `handleAddToCalendar` (`:59–62`) sets a 3 s toast “Calendar event added”. No file, no provider, no API. | Download or deliver ICS from `smartmatch_domain.ics` for a *resolved* event; refuse `unresolved` (F-003). | Domain ICS exists; **no HTTP**. Calendar provider gated G5. | Command that returns ICS bytes; UI labels “ICS download”. Never toast success without a blob. | M |
