...
| Pipeline funnel — Matched → Contacted → Confirmed → Attended → Member Inquiry | **Schema and read path only.** `pipeline_record` exists (migration `0011`) and the five ADR-0011 metrics read it through `pipeline_funnel_rows_v1`; the only application write path is the outreach handler's Contacted advance (migration `0021`, card L5), and it fires **only when a send command names an existing journey** — so the funnel still measures a real zero until something creates `pipeline_record` rows | A `pipeline_record` *creation* path. The outreach slice deliberately does not invent one: an email to a professional is not a student's journey, and manufacturing a row to have something to advance would inflate every funnel metric |
