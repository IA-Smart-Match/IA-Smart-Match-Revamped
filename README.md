...
| Authenticated student event access — published-event browse, the caller's own agenda, registration/cancellation, and calendar download | `services/api/.../routers/student_events.py`, `services/api/.../routers/calendar.py`, `smartmatch_persistence.student_events` | `tests/contract/test_student_events_api.py` + `tests/contract/test_calendar_ics.py` |
