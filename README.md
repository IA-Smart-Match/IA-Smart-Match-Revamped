...
| Tenant-safe schema, enforced by composite keys — 43 Alembic revisions, head `0043_exercise_event_exploratory` | `db/migrations` | 11 integration |
...
| Authenticated student event access — published-event browse, the caller's own agenda, registration/cancellation, and calendar download | `services/api/.../routers/student_events.py`, `services/api/.../routers/calendar.py`, `smartmatch_persistence.event_registration` | `tests/contract/test_student_events_api.py` + `tests/contract/test_calendar_ics.py` |
| Attendance evidence and server-owned points — coordinator entry writes attendance, credits its derived ledger entry in the same transaction, and balances are server-side folds rather than stored/browser totals | `services/api/.../routers/attendance.py`, `smartmatch_persistence.rewards`, `db/migrations` | `tests/contract/test_attendance_api.py` + `tests/integration/test_rewards_repository.py` |
| Funded rewards catalog and redemption operations — students read only funded, owned items and their folded balance; request/decision routes preserve approval and fulfilment transitions | `services/api/.../routers/rewards.py`, `smartmatch_persistence.rewards` | `tests/integration/test_rewards_api.py` + `tests/integration/test_redemption_durability.py` |
| Per-event feedback QR — administrator-managed stable redirect, data-minimised open count, and no third-party QR service | `services/api/.../routers/manual_events.py`, `smartmatch_persistence.manual_events` | `tests/unit/test_manual_events.py` + `tests/contract/test_manual_events_api.py` |
