...
const funnelPalette = ["#2F036C", "#743EB7", "#9A64CF", "#B69CE9", "#DFD6F1"];
...
export function Dashboard() {
  ...
  const [discoveryFeedOpen, setDiscoveryFeedOpen] = useState(true);
    ...
    async function load() {
      try {
        ...
        if (active) {
          ...
          setFeedbackStats(MOCK_FEEDBACK_STATS);
          setIsMockData(true);
          setError(null); // The status badge already signals the fallback state
        }
      } finally {
        if (active) {
          setLoading(false);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-start">
        ...
        <div className="order-1 rounded-2xl border border-[#DFD6F1] bg-white p-6 shadow-sm lg:order-2 lg:max-h-[360px] lg:overflow-hidden">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="text-xl font-semibold text-gray-900">Discovery feed</h3>
              <p className="text-sm text-gray-600">
                Coordinator notifications derived from the current dataset.
              </p>
            </div>
            <button
              type="button"
              onClick={() => setDiscoveryFeedOpen((open) => !open)}
              aria-expanded={discoveryFeedOpen}
              aria-controls="dashboard-discovery-feed"
              className="rounded-lg border border-[#DFD6F1] p-2 text-[#2F036C] transition-colors hover:bg-[#FBF8FF]"
              title={discoveryFeedOpen ? "Collapse discovery feed" : "Expand discovery feed"}
            >
              {discoveryFeedOpen ? (
              ...
              <span className="sr-only">
                {discoveryFeedOpen ? "Collapse discovery feed" : "Expand discovery feed"}
              </span>
            </button>
          </div>

          {discoveryFeedOpen ? (
            <div id="dashboard-discovery-feed" className="mt-4 max-h-[260px] space-y-3 overflow-y-auto pr-2">
              {discoveryFeed.map((item) => {
                const priorityStyles: Record<string, string> = {
                  action: "border-red-200 bg-red-50/70",
                  defer: "border-amber-200 bg-amber-50/70",
                  watch: "border-emerald-200 bg-emerald-50/70",
                };
                const stampStyles: Record<string, string> = {
                  action: "bg-red-100 text-red-800",
                  defer: "bg-amber-100 text-amber-800",
                  watch: "bg-emerald-100 text-emerald-800",
                };

                return (
                  <div
                    key={item.title}
                    className={`rounded-2xl border p-4 shadow-sm ${priorityStyles[item.priority]}`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <p className="font-semibold text-gray-900">{item.title}</p>
                      <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] ${stampStyles[item.priority]}`}>
                        {item.stamp}
                      </span>
                    </div>
      ...
      <div className="rounded-2xl border border-[#DFD6F1] bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <ShieldCheck className="h-5 w-5 text-[#2F036C]" />
            <div>
          ...
          <Link to="/calendar" className="shrink-0 text-xs font-medium text-[#2F036C] hover:underline">
            View calendar →
          </Link>
        </div>

        <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl border border-[#DFD6F1] bg-[#FBF8FF] p-4">
          ...
          <div className="rounded-2xl border border-[#DFD6F1] bg-[#FBF8FF] p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-gray-500">Avg invitation pace</p>
          ...
          <div className="rounded-2xl border border-[#DFD6F1] bg-[#FBF8FF] p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-gray-500">Speakers needing a break</p>
      ...
      <div className="rounded-2xl border border-[#DFD6F1] bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-2">
            <MessageSquareHeart className="h-5 w-5 text-[#2F036C]" />
            <div>
          ...
          <div className="flex items-center gap-3">
            <div className="rounded-full border border-[#DFD6F1] bg-[#FBF8FF] px-3 py-1 text-xs font-medium text-[#2F036C]">
              {feedbackStats.total_feedback} feedback rows
            </div>
            <Link to="/ai-matching" className="text-xs font-medium text-[#2F036C] hover:underline">
              View matches →
            </Link>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[1fr_0.95fr]">
          <div className="rounded-2xl border border-[#DFD6F1] bg-[#FBF8FF] p-4">
            ...
            {feedbackStats.trend.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-[#D8CDED] bg-white p-6 text-sm text-gray-600">
              ...
              <ResponsiveContainer width="100%" height={220}>
                ...
                >
                  <CartesianGrid strokeDasharray="3 3" stroke="#E7DDF6" />
          ...
          <div className="rounded-2xl border border-[#DFD6F1] bg-[#FBF8FF] p-4">
            <div className="mb-3 flex items-center gap-2">
              <SlidersHorizontal className="h-4 w-4 text-[#2F036C]" />
              ...
              {feedbackStats.recommended_adjustments.length === 0 ? (
                ...
                feedbackStats.recommended_adjustments.slice(0, 4).map((adjustment) => (
                  <div
                    ...
                    className="rounded-2xl border border-[#DFD6F1] bg-white p-4 shadow-sm"
                  >
                    <div className="flex items-center justify-between gap-3">
                      <p className="font-semibold text-gray-900">
                        {formatFactorName(adjustment.factor)}
                      </p>
                      <span className="text-sm font-semibold text-[#2F036C]">
                        {adjustment.delta > 0 ? "+" : ""}
      ...
      <div className="rounded-2xl border border-[#DFD6F1] bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                <MapPinned className="h-5 w-5 text-[#2F036C]" />
              </div>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
            {regionalPulse.length ? (
              regionalPulse.map((region) => (
                <div
                  key={region.region}
                  className="rounded-2xl border border-[#DFD6F1] bg-white p-5 shadow-sm"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                    ...
                    <span className="rounded-full border border-[#DFD6F1] bg-white px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-[#2F036C]">
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    <div className="h-2 rounded-full bg-white/80">
                      <div
                        className="h-2 rounded-full bg-[#2F036C]"
              ...
              <div className="rounded-2xl border border-dashed border-[#D8CDED] bg-[#FBF8FF] p-8 text-sm text-gray-600 lg:col-span-2">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-[#DFD6F1] bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/pipeline" className="text-xs font-medium text-[#2F036C] hover:underline">
              View pipeline →
            </Link>
          </div>
        ...
        <div className="rounded-2xl border border-[#DFD6F1] bg-white p-6 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={eventVolume}>
              ...
              <Tooltip />
              <Bar dataKey="matches" fill="#743EB7" radius={[8, 8, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[#DFD6F1] bg-white p-6 shadow-sm">
        <div className="mb-6 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-[#2F036C]" />
            <h3 className="text-lg font-semibold text-gray-900">Top Recommended Matches</h3>
          </div>
          <Link
            to="/ai-matching"
            className="flex items-center gap-1 text-sm font-medium text-[#2F036C] transition-colors hover:text-[#5B219F]"
          >
            View All
        ...
        <div className="space-y-4">
          {topMatches.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-[#D8CDED] bg-[#FBF8FF] p-8 text-center text-gray-600">
            ...
            topMatches.map((match) => (
              <div
                ...
                className="flex items-center justify-between gap-4 rounded-2xl border border-[#DFD6F1] bg-white p-4 shadow-sm transition-shadow hover:shadow-md"
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <p className="font-semibold text-gray-900">{match.name}</p>
                    <span className="rounded-full bg-[#EEE9FB] px-2 py-0.5 text-xs text-[#2F036C]">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    ...
                    <p className="text-2xl font-semibold text-[#2F036C]">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-xl bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow-sm transition-colors hover:bg-[#5B219F] disabled:cursor-not-allowed disabled:opacity-50"
                  >
