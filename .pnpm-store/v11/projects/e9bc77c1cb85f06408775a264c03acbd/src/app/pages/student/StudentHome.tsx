            ...
            >
              ...
              <ul className="max-h-[min(22rem,50vh)] divide-y divide-border/60 overflow-y-auto px-1 py-1">
                {rewardPreview.map((item) => {
                  ...
                  const pointsAway = Math.max(item.pointsCost - totalPoints, 0);
                  return (
                    <li key={item.id} className="px-3 py-2.5">
                      <div>
                        <p className="text-sm font-medium leading-snug text-foreground">{item.title}</p>
                        <p className={`mt-1 text-xs font-semibold ${unlocked ? "text-primary" : "text-muted-foreground"}`}>
                          {unlocked ? "Ready to redeem" : `${pointsAway.toLocaleString()} points away`}
                          {" · "}
                          {item.pointsCost.toLocaleString()} points to redeem
                        </p>
                      </div>
