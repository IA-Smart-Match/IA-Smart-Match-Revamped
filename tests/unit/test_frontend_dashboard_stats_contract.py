...
def test_the_statistics_surface_summarises_events_without_counting_them() -> None:
    ...
    assert "fetchManualEvents" in code, (
        "the hosted-events panel must read the canonical manual-event listing"
    )
    assert "events.data.total" in code, "the displayed count must come from the server response"
    for forbidden in ("events.length", "events.data.data.length", "listing.events.length"):
        assert forbidden not in code, (
...
def test_the_outreach_panel_is_replaced_by_internal_handoffs() -> None:
    """The dashboard tracks status while email remains outside Smart Match."""
    code = _code_only(STATS_PAGE.read_text(encoding="utf-8"))

    assert "Speaker handoffs" in code
    assert "Email is" in code and "outside Smart Match" in code
    assert "fetchOutreachDrafts" not in code
    assert "fetchOutreachSends" not in code
...
def test_the_surface_claims_no_absence_for_a_dataset_it_now_reads() -> None:
    ...
    assert 'to="/coordinator-portal/meetings"' in code
    assert 'dataset="Meeting bookings"' not in code
