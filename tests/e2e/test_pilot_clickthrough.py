...
def test_25_student_feedback_reaches_a_connector_only_as_an_aggregate(
    ...
    **OQ-CBA-053** is pinned at the end: no rating is a scoring input. The
    ...
    # The unit-level pool, the Connector dashboard's read. On this appliance no
    # rating can be submitted at all, so the honest answer is a suppression --
    # and a suppression that published a zero would be the ADR-0011 defect on a
    # second surface.
    ...
    assert unit_aggregate["suppressed"] is True, (
        "the unit aggregate published a number on an appliance where no rating "
        "was ever submitted; the only honest answer over zero stored ratings is "
        "a suppression"
    )
