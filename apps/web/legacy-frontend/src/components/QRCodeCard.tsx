...
import { Download, ExternalLink, QrCode, Sparkles, ScanSearch } from "lucide-react";
import { toDataURL, toString as qrToString } from "qrcode";

import type { FeedbackQrAsset, QrCodeAsset } from "@/lib/api";

/**
 * `QRCodeCard` renders two unrelated QR surfaces behind one export name, kept
 * this way so every existing caller (`Volunteers.tsx`, `Outreach.tsx` — the
 * referral/scan QR) continues to render unchanged with no prop or import
 * edits, while `Events.tsx` gets the manual-event feedback QR (local
 * generation via `qrcode`, no third-party QR service, and reporting only QR
 * *opens* — never a scan-to-conversion metric, which this surface has no
 * server-reported number for).
 *
 * Pass `variant="feedback"` for the new surface; omit it (or pass
 * `variant="referral"`) for the original one.
 */
type ReferralQRCodeCardProps = {
  variant?: "referral";
...
type QRCodeCardProps = ReferralQRCodeCardProps | FeedbackQRCodeCardProps;

export function QRCodeCard(props: QRCodeCardProps) {
  if (props.variant === "feedback") {
    return <FeedbackQrCard {...props} />;
  }
  return <ReferralQrCard {...props} />;
}

// ---------------------------------------------------------------------------
// Feedback QR (manual events) — local rendering, external destination only.
// ---------------------------------------------------------------------------

function feedbackHostname(value: string) {
  try {
...
function FeedbackQrCard({ asset, loading = false, error, onSave }: FeedbackQRCodeCardProps) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      qrToString(redirectUrl, { ...options, type: "svg" }),
  ...
  return (
    <section
      ...
      aria-labelledby="feedback-qr-heading"
    >
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 id="feedback-qr-heading" className="text-xl font-semibold text-foreground">
            External feedback QR
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Add your organization's feedback-form link. This QR code opens that external
            website — Smart Match does not host, inspect, or receive responses from it, and
            only counts how many times the QR opens.
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[220px_1fr]">
        <div className="flex aspect-square items-center justify-center rounded-2xl border border-dashed border-border bg-muted/40 p-4">
          {preview ? (
            <img
              src={preview}
              alt="QR code that opens the external event feedback form"
              className="h-full w-full object-contain"
            />
          ) : (
            <p className="text-center text-sm text-muted-foreground">
              Save a valid HTTPS form link to generate the QR code.
            </p>
          )}
        </div>
        <div className="space-y-4">
          <div>
            <label htmlFor="feedback-url" className="mb-2 block text-sm font-medium">
              Feedback form URL
            </label>
            <input
              id="feedback-url"
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              HTTPS links only. This QR opens an external website that Smart Match does not
              host, inspect, or receive responses from.
            </p>
          ...
          {savedMessage ? (
            <p role="status" className="rounded-xl bg-primary/5 px-4 py-3 text-sm text-primary">
              {savedMessage}
            </p>
          ) : null}
          ...
          {asset ? (
            <div className="rounded-xl bg-muted px-4 py-3 text-sm">
              <p className="font-medium text-foreground">
                Destination: {feedbackHostname(asset.destination_url)}
              </p>
              ...
              <a
                href={asset.destination_url}
                target="_blank"
                rel="noreferrer"
                className="mt-2 inline-flex items-center gap-1 font-medium text-primary hover:underline"
              >
                Check external website <ExternalLink className="h-3.5 w-3.5" aria-hidden="true" />
              </a>
            </div>
          ) : null}
          ...
          <div className="flex flex-wrap gap-3">
            <button
              type="button"
              disabled={loading || !destinationUrl.trim() || !confirmed}
              onClick={() => {
                setSavedMessage("");
            ...
            <button
              ...
              onClick={download}
              className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2.5 text-sm font-semibold text-foreground hover:bg-muted disabled:opacity-60"
            >
...
// ---------------------------------------------------------------------------
// Referral QR (speaker/event scan tracking) — unchanged from before this file
// grew a second variant. See callers in `Volunteers.tsx` and `Outreach.tsx`.
// ---------------------------------------------------------------------------
...
function resolveFilename(asset: QrCodeAsset) {
  const base =
    slugify(
      `${asset.speaker_name || "speaker"}-${asset.event_name || "event"}-${asset.referral_code || "qr"}`,
    );
  const usesPng =
