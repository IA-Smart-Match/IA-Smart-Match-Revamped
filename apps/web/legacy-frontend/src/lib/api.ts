import {
  localCoordinatorEvents,
  localCoordinatorMeetings,
  localCoordinatorProfile,
  localCoordinatorThreads,
  localCrawlerResults,
  localCrawlerStatus,
  localData,
  localRankedMatches,
  localStudentConnections,
  localStudentNudge,
  localStudentRecommendations,
  localStudentProfile,
  localStudentRegistrations,
  localVolunteerAssignments,
  localVolunteerProfile,
} from "./localBackend";
...
async function withLocalFallback<T>(request: Promise<T>, fallback: T): Promise<T> {
  try {
    return await request;
  } catch {
    // The foundation API intentionally does not expose the legacy routes yet.
    // Keep the UI usable from the checked-in workspace data until those routes
    // are available, while still preferring live responses whenever possible.
    return fallback;
  }
}
...
function recoveryLabel(status: RecoveryStatus): string {
  switch (status) {
    case "Available":
      return "Open to invitations";
    case "Needs Rest":
      return "Recently invited often";
    case "Rest Recommended":
      return "Give them a break";
    default:
      return "Availability not yet known";
  }
}
...
function normalizeCalendarAssignment(record: Record<string, unknown>, index: number): CalendarAssignmentSummary {
  ...
  return {
    ...
    recovery_status: recoveryStatus,
    recovery_label: recoveryLabel(recoveryStatus),
...
function normalizeRankedMatch(payload: Partial<RankedMatch> & Record<string, unknown>): RankedMatch {
  ...
  return {
    ...
    recovery_label: recoveryLabel(
      normalizeRecoveryStatus(payload.recovery_status ?? payload.recoveryState, volunteerFatigue),
    ),
...
export async function fetchSpecialists(): Promise<WithSource<Specialist[]>> {
  const raw = await withLocalFallback(requestJson<unknown>("/api/data/specialists"), localData.specialists);
...
export async function fetchEvents(): Promise<WithSource<CppEvent[]>> {
  const raw = await withLocalFallback(requestJson<unknown>("/api/data/events"), localData.events);
  const payload = toRecordArray(raw);
  const rawSource = payload[0]?.source;
  const source: "live" | "demo" | "csv" = rawSource === "demo" ? "demo" : rawSource === "csv" ? "csv" : "live";
...
export async function fetchPipeline(): Promise<WithSource<PipelineRecord[]>> {
  const raw = await withLocalFallback(requestJson<unknown>("/api/data/pipeline"), localData.pipeline);
  const payload = toRecordArray(raw);
  const rawSource = payload[0]?.source;
  const source: "live" | "demo" | "csv" = rawSource === "demo" ? "demo" : rawSource === "csv" ? "csv" : "live";
...
export async function fetchCalendar(): Promise<CalendarRecord[]> {
  return withLocalFallback(
    requestJson<CalendarRecord[]>("/api/data/calendar"),
    localData.calendarEvents.map((event) => ({
      "IA Event Date": event.event_date,
      Region: event.region,
      "Nearby Universities": event.nearby_universities.join(", "),
      "Suggested Lecture Window": event.suggested_lecture_window,
    })),
  );
}

export async function fetchCalendarEvents(): Promise<WithSource<CalendarEventSummary[]>> {
  const payload = await withLocalFallback(requestJson<unknown>("/api/calendar/events"), localData.calendarEvents);
...
export async function fetchCalendarAssignments(): Promise<WithSource<CalendarAssignmentSummary[]>> {
  const payload = await withLocalFallback(requestJson<unknown>("/api/calendar/assignments"), localData.calendarAssignments);
  const rows = toRecordArray(payload);
  const rawSource = (rows[0] as Record<string, unknown>)?.source;
  const source: "live" | "demo" | "csv" = rawSource === "demo" ? "demo" : rawSource === "csv" ? "csv" : "live";
...
export async function fetchContacts(): Promise<PocContact[]> {
  return withLocalFallback(requestJson<PocContact[]>("/api/data/contacts"), []);
}
...
export async function fetchQrStats(): Promise<WithSource<QrStatsSummary>> {
  const payload = await withLocalFallback(requestJson<Record<string, unknown>>("/api/qr/stats"), localData.qrStats as unknown as Record<string, unknown>);
...
export async function fetchFeedbackStats(): Promise<WithSource<FeedbackStatsSummary>> {
  const payload = await withLocalFallback(requestJson<Record<string, unknown>>("/api/feedback/stats"), localData.feedbackStats as unknown as Record<string, unknown>);
  const rawSource = payload?.source;
  const source: "live" | "demo" | "csv" = rawSource === "demo" ? "demo" : rawSource === "csv" ? "csv" : "live";
...
export async function submitFeedback(
  ...
  try {
    ...
    return {
      feedback: { ...input, recorded_at: new Date().toISOString(), source: "local" },
      optimizer_snapshot: {
        timestamp: new Date().toISOString(),
        total_feedback: localData.feedbackStats.total_feedback + 1,
        accepted: localData.feedbackStats.accepted + (input.decision === "accept" ? 1 : 0),
        declined: localData.feedbackStats.declined + (input.decision === "decline" ? 1 : 0),
        acceptance_rate: localData.feedbackStats.acceptance_rate,
        pain_score: localData.feedbackStats.pain_score,
        weights: localData.feedbackStats.current_weights,
        baseline_weights: localData.feedbackStats.default_weights,
        adjustments: localData.feedbackStats.recommended_adjustments,
      },
    };
  }
}
...
export async function generateQrAsset(
  ...
  let asset: QrCodeAsset;
  try {
    const payload = await requestJson<unknown>("/api/qr/generate", {
      method: "POST",
      body: JSON.stringify({ speaker_name: speakerName, event_name: eventName }),
    });
    asset = normalizeQrCodeAsset(payload);
  } catch {
    const referralCode = `${speakerName}-${eventName}`
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/(^-|-$)/g, "");
    asset = normalizeQrCodeAsset({
      referral_code: referralCode,
      speaker_name: speakerName,
      event_name: eventName,
      generated_at: new Date().toISOString(),
      scan_url: `${window.location.origin}/check-in/${referralCode}`,
    });
  }
  if (
    !asset.referral_code &&
...
export async function rankSpeakers(
  ...
  const payload = await withLocalFallback(requestJson<Array<Record<string, unknown>>>("/api/matching/rank", {
    method: "POST",
    body: JSON.stringify({
      event_name: eventName,
      limit,
  ...
  }), localRankedMatches.slice(0, limit) as unknown as Array<Record<string, unknown>>);
...
export async function scoreSpeaker(
  ...
  return withLocalFallback(requestJson<MatchScore>("/api/matching/score", {
    method: "POST",
    body: JSON.stringify({
      speaker_name: speakerName,
      event_name: eventName,
    ...
    const match = localRankedMatches.find((entry) => entry.name === speakerName && entry.event_name === eventName);
    return match
      ? {
          speaker_name: match.name,
          event_name: match.event_name,
          total_score: match.total_score,
          volunteer_fatigue: match.volunteer_fatigue,
          factor_scores: match.factor_scores,
          weighted_factor_scores: match.weighted_factor_scores,
        }
      : {
          speaker_name: speakerName,
          event_name: eventName,
...
export async function generateEmail(
  ...
  return withLocalFallback(requestJson<OutreachEmailResponse>("/api/outreach/email", {
    method: "POST",
    body: JSON.stringify({
      speaker_name: speakerName,
      event_name: eventName,
    ...
    email: `Subject: Invitation to support ${eventName}\n\nHello ${speakerName},\n\nWe would love to explore your participation in ${eventName}. Please let us know if the proposed opportunity aligns with your schedule and interests.\n\nBest,\nIA West`,
    email_data: {
      subject_line: `Invitation to support ${eventName}`,
      greeting: `Hello ${speakerName},`,
      body: `We would love to explore your participation in ${eventName}. Please let us know if the proposed opportunity aligns with your schedule and interests.`,
      closing: "Best,\nIA West",
      full_email: `Hello ${speakerName},\n\nWe would love to explore your participation in ${eventName}. Please let us know if the proposed opportunity aligns with your schedule and interests.\n\nBest,\nIA West`,
    },
    voice: options?.voice ?? "ia_west_chapter",
  });
}
...
export async function generateIcs(
  ...
  return withLocalFallback(requestJson<{ ics_content: string }>("/api/outreach/ics", {
    method: "POST",
    body: JSON.stringify({
      event_name: eventName,
    ...
    ics_content: `BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//Insights Association//Smart Match//EN\nBEGIN:VEVENT\nSUMMARY:${eventName}\nDTSTART:${(eventDate ?? "20260415").replace(/-/g, "")}T090000\nDTEND:${(eventDate ?? "20260415").replace(/-/g, "")}T100000\nLOCATION:${location ?? "University campus"}\nDESCRIPTION:${description ?? "Insights Association university engagement"}\nEND:VEVENT\nEND:VCALENDAR`,
  });
}
...
export async function initiateWorkflow(
  ...
  return withLocalFallback(requestJson<WorkflowResponse>("/api/outreach/workflow", {
    method: "POST",
    body: JSON.stringify({
      speaker_name: speakerName,
      event_name: eventName,
    ...
    email_data: {
      subject_line: `Invitation to support ${eventName}`,
      greeting: `Hello ${speakerName},`,
      body: `We would love to explore your participation in ${eventName}.`,
      closing: "Best,\nIA West",
      full_email: `Hello ${speakerName},\n\nWe would love to explore your participation in ${eventName}.\n\nBest,\nIA West`,
    },
    ics_content: "",
    pipeline_updated: true,
    steps: { email: { status: "ok" }, ics: { status: "ok" }, pipeline: { status: "ok" } },
    dispatch_mode: "local",
  });
}
...
export async function fetchCourses(): Promise<CppCourse[]> {
  const raw = await withLocalFallback(requestJson<unknown>("/api/data/courses"), []);
...
export async function rankSpeakersForCourse(
  ...
  const payload = await withLocalFallback(requestJson<Array<Record<string, unknown>>>("/api/matching/rank-for-course", {
    method: "POST",
...
export async function startCrawl(): Promise<{ status: string }> {
  return withLocalFallback(requestJson<{ status: string }>("/api/crawler/start", {
    method: "POST",
  }), { status: "accepted" });
}

export async function fetchCrawlerResults(): Promise<CrawlerResultsResponse> {
  return withLocalFallback(requestJson<CrawlerResultsResponse>("/api/crawler/results"), localCrawlerResults());
}

export async function clearCrawlerResults(): Promise<{ deleted: number; status: string }> {
  return withLocalFallback(requestJson<{ deleted: number; status: string }>("/api/crawler/results", {
    method: "DELETE",
  }), { deleted: 0, status: "cleared" });
}
...
export async function fetchCrawlerStatus(): Promise<CrawlerStatusResponse> {
  return withLocalFallback(requestJson<CrawlerStatusResponse>("/api/crawler/status"), localCrawlerStatus());
}
...
export async function fetchStudentProfile(studentId: string): Promise<StudentProfile & { source: string }> {
  try {
    const res = await fetch(`${API_BASE}/portals/students/${encodeURIComponent(studentId)}`);
    if (!res.ok) throw new Error(`Student not found: ${studentId}`);
    return res.json();
  } catch {
    return localStudentProfile(studentId);
  }
}
...
export async function fetchStudentRegistrations(studentId: string): Promise<{ data: StudentRegistration[]; total: number; source: string }> {
  try {
    const res = await fetch(`${API_BASE}/portals/students/${encodeURIComponent(studentId)}/registrations`);
    if (!res.ok) throw new Error("Registrations not found");
    return res.json();
  } catch {
    return localStudentRegistrations(studentId);
  }
}
...
export async function fetchStudentConnectionSuggestions(
  ...
  for (const endpoint of endpoints) {
    ...
    return localStudentConnections(studentId);
  }
...
export async function fetchStudentRecommendations(studentId: string): Promise<{ recommendations: (CalendarEventSummary & { is_recommended: boolean })[]; source: string }> {
  try {
    const res = await fetch(`${API_BASE}/portals/students/${encodeURIComponent(studentId)}/recommendations`);
    if (!res.ok) throw new Error("Recommendations failed");
    return res.json();
  } catch {
    return localStudentRecommendations(studentId);
  }
}
...
export async function fetchStudentNudge(
  ...
  try {
    const res = await fetch(`${API_BASE}/portals/students/${encodeURIComponent(studentId)}/nudge`);
    if (res.status === 404) return null;
    if (!res.ok) throw new Error(`Nudge failed: ${res.status}`);
    return res.json();
  } catch {
    return localStudentNudge(studentId);
  }
}

export async function fetchCoordinatorProfile(coordinatorId: string): Promise<EventCoordinator & { source: string }> {
  try {
    const res = await fetch(`${API_BASE}/portals/event-coordinators/${encodeURIComponent(coordinatorId)}`);
    if (!res.ok) throw new Error("Coordinator not found");
    return res.json();
  } catch {
    return localCoordinatorProfile(coordinatorId);
  }
}
...
export async function fetchCoordinatorThreads(coordinatorId: string): Promise<{ data: OutreachThread[]; total: number; source: string }> {
  try {
    const res = await fetch(`${API_BASE}/portals/event-coordinators/${encodeURIComponent(coordinatorId)}/threads`);
    if (!res.ok) throw new Error("Threads failed");
    return res.json();
  } catch {
    return localCoordinatorThreads(coordinatorId);
  }
}
...
export async function fetchCoordinatorMeetings(coordinatorId: string): Promise<{ data: MeetingBooking[]; total: number; source: string }> {
  try {
    const res = await fetch(`${API_BASE}/portals/event-coordinators/${encodeURIComponent(coordinatorId)}/meetings`);
    if (!res.ok) throw new Error("Meetings failed");
    return res.json();
  } catch {
    return localCoordinatorMeetings(coordinatorId);
  }
}
...
export async function fetchCoordinatorEvents(coordinatorId: string): Promise<{ data: (CalendarEventSummary & { staffing_open: boolean })[]; total: number; source: string }> {
  let res: Response;
  try {
    res = await fetch(`${API_BASE}/portals/event-coordinators/${encodeURIComponent(coordinatorId)}/events`);
    if (!res.ok) throw new Error(`Coordinator events failed: ${res.status}`);
  } catch {
    return localCoordinatorEvents();
  }
  ...
  return {
    data: data.map((event) => ({ ...event, staffing_open: Boolean(event.staffing_open) })),
...
export async function fetchVolunteerProfile(
  ...
  try {
    ...
    return localVolunteerProfile(volunteerId);
  }
}
...
export async function fetchVolunteerAssignments(
  ...
  try {
    ...
    return localVolunteerAssignments();
  }
}
