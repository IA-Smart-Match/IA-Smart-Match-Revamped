 ...
 */
function StudentFeedbackPointer({ feedback }: { feedback: Loaded<UnitFeedbackSummary> }) {
...
export function CoordinatorHome() {
  ...
  const [events, setEvents] = useState<Loaded<{ data: ManualEvent[]; total: number }>>(PENDING);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const listing = await fetchManualEvents(unitId, "published");
    ...
    <div className="space-y-6">
      ...
      {unitId === null ? (
        ...
        <>
          ...
          <StudentFeedbackPointer feedback={feedback} />
          <section className="rounded-2xl border border-border p-6">
            <div className="flex items-start gap-2">
              ...
              />
              <div>
                <h2 className="font-semibold text-foreground">Published events</h2>
                {events.error ? (
                  ...
                  <p role="status" className="mt-2 text-sm text-muted-foreground">
                    {events.settled ? "The event listing returned nothing." : "Loading events…"}
                  </p>
                ) : (
                  <p className="mt-2 text-sm text-muted-foreground">
                    {events.data.total} published {events.data.total === 1 ? "event" : "events"}.
                  </p>
                )}
                <Link
                  ...
                  to="/coordinator-portal/events"
                >
                  Review events and run Smart Match
                </Link>
              </div>
            </div>
          </section>
          <section className="rounded-2xl border border-border p-6">
            <h2 className="font-semibold text-foreground">Speaker handoffs</h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Review shared invitation status, notes, confirmation, and attendance. Email is
              handled outside Smart Match.
            </p>
            <Link
              className="mt-3 inline-block font-medium text-foreground underline underline-offset-4"
              to="/coordinator-portal/outreach"
            >
              Open speaker handoffs
            </Link>
          </section>
        </>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Link className="rounded-2xl border border-border p-6" to="/coordinator-portal/meetings">
          <h2 className="font-semibold text-foreground">Meeting records</h2>
          <p className="mt-2 text-sm text-muted-foreground">Review or record internally arranged meetings.</p>
        </Link>
        <Link className="rounded-2xl border border-border p-6" to="/coordinator-portal/review-queue">
          <h2 className="font-semibold text-foreground">Review queue</h2>
          <p className="mt-2 text-sm text-muted-foreground">Open the server-backed queue of items awaiting review.</p>
        </Link>
      </div>
    </div>
  );
