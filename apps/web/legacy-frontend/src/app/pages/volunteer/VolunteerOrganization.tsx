...
export function VolunteerOrganization() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setState({ status: "ready", own: await fetchOwnHostOrganization(unitId) });
    } catch (cause) {
      // The 404 is the "none" state — the honest answer to the question this
      // URL asks — and is branched on the error's code, never its text.
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      // so the card does not claim which of the two the call was.
      setState({ status: "ready", own: await upsertOwnHostOrganization(unitId, payload) });
      setEditing(false);
    } catch (cause) {
