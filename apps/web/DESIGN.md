...
- One login, two roles (T6b-5): an Event Host who is also a Speaker signs in once and holds both portals. The Event Host and Speaker shells then show a "Switch portal" button — above the profile block on desktop, in the mobile header's right-hand slot on phones — that discloses a list of links to each portal's home, named by the server's `display_name`, with `aria-current="true"` on the portal in view. It is a disclosure of links, not a menu: Escape closes it and returns focus to the button, and it renders nothing for a login with one portal. The Speaker Connector and Student shells never show it. Choosing a portal is remembered on this browser (the portal id only) so `/` opens it next time; sign-out forgets it. The sign-in screen stays two fields with no portal chooser.
