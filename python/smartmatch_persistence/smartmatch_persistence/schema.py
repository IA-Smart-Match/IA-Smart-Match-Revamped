...
event = sa.Table(
    ...
    # Migration 0036. The Event Host's **own** organization -- the club,
    # department or office they say they are asking on behalf of -- which is a
    # different thing from `host_org_unit_id` above: that one is the
    # institution's org unit, the department the request is filed under and
    # authorized against. Nullable, and the NULL means **no organization was
    # recorded**, never "the host has none" and never "look it up from the
    # filer": reading today's affiliation back onto a request filed before it
    # existed is a reconstruction indistinguishable from a recorded fact
    # (ADR-0011 rule 1), which is the same reason 0033's column was not
    # backfilled either. Nothing authorizes on it -- enforcement stays per-user
    # through `filed_by_user_id` until a coordinator grants membership.
    sa.Column("host_organization_id", _UUID, nullable=True),
    sa.Column("created_at", _TS, nullable=False, server_default=sa.text("now()")),
    sa.Column("updated_at", _TS, nullable=False, server_default=sa.text("now()")),
    ...
    # Migration 0036, the mirror of the constraint above. An extracted event
    # has no host organization for the same reason it has no filer: a crawl is
    # nobody's affiliation. Partial by construction, so 0036 could add it to a
    # populated table -- every existing row satisfies the IS NULL arm without
    # being read.
    sa.CheckConstraint(
        "host_organization_id IS NULL OR origin = 'coordinator_entry'",
        name="ck_event_host_organization_manual_origin",
    ),
    # Migration 0036. Composite, like every cross-table reference in this
    # schema: a single-column key would accept an organization from another
    # tenant. RESTRICT, so deleting an organization a request was filed under
    # is an error rather than a silent un-stamping.
    sa.ForeignKeyConstraint(
        ["tenant_id", "host_organization_id"],
        ["host_organization.tenant_id", "host_organization.id"],
        ondelete="RESTRICT",
        name="fk_event_host_organization",
    ),
)


# ---------------------------------------------------------------------------
# The Event Host's own organization (migration 0036). PLAN v2 finding 10 and
# owner decision 4: modelled now, enforcement per-user. See the migration's
# docstring for the reasoning behind every constraint mirrored below.
#
# Read the second sentence again before building on these tables. **Nothing
# authorizes on them.** Whose Speaker Requests a caller may read is still
# decided by `event.filed_by_user_id` and by nothing else; an organization is
# a description a host typed about themselves, not a permit, and it stays that
# way until a coordinator can grant membership (the `granted_by_user_id`
# column is where that day's distinction will be stored).
# ---------------------------------------------------------------------------

host_organization = sa.Table(
    "host_organization",
    METADATA,
    sa.Column("id", _UUID, primary_key=True),
    sa.Column("tenant_id", _UUID, nullable=False),
    # The institution's org unit this organization files into -- the same unit
    # `event.host_org_unit_id` names. An organization is scoped to one unit
    # rather than to the tenant because the routes that read and write it are
    # unit-scoped, and a tenant-wide record would be reachable from a unit
    # nobody authorized the caller against.
    sa.Column("unit_id", _UUID, nullable=False),
    sa.Column("name", sa.Text, nullable=False),
    # Nullable and undefaulted, all four. A host who has not said which
    # department they sit in has not said it; an empty string would be a
    # statement they did not make.
    sa.Column("department", sa.Text, nullable=True),
    sa.Column("default_location", sa.Text, nullable=True),
    # Free text about who to reach for logistics. Deliberately **not** a
    # `contact_channel`: no outreach path reads it, nothing treats it as an
    # address, and it grants no consent.
    ...
    # What the member table's composite key references, so a member row cannot
    # disagree with its organization about which unit it is in.
    sa.UniqueConstraint("tenant_id", "id", "unit_id", name="uq_host_organization_tenant_id_unit"),
    sa.ForeignKeyConstraint(
        ["tenant_id", "unit_id"],
        ["org_unit.tenant_id", "org_unit.id"],
        ondelete="RESTRICT",
    ...
    sa.Index("ix_host_organization_unit", "tenant_id", "unit_id", "name"),
    # One organization per name per unit, case-folded. Two hosts typing
    # "Accounting Society" into the same department are describing one club,
    # and two rows for it would split its requests with nothing to say they
    # belong together. Functional rather than a plain UNIQUE so the two
    # spellings collide, which is the point.
    sa.Index(
...
host_organization_member = sa.Table(
    "host_organization_member",
    METADATA,
    sa.Column("organization_id", _UUID, nullable=False),
    sa.Column("user_id", _UUID, nullable=False),
    sa.Column("tenant_id", _UUID, nullable=False),
    # The unit this member's organization files into, carried here so the
    # unit-scoped host routes can answer without a join and, more importantly,
    # kept honest by the composite key to `uq_host_organization_tenant_id_unit`
    # -- a member row whose unit disagreed with its organization's would be
    ...
    # value names the coordinator who granted the membership. Nothing writes
    # one yet; the column exists so that when something does, a granted row is
    # distinguishable from an asserted one rather than indistinguishable after
    # the fact (ADR-0011 rule 1).
    ...
    # This is what makes "the caller's organization" a well-defined phrase on
    # the host routes: two simultaneous affiliations would be two answers to
    # one question.
    sa.UniqueConstraint("user_id", name="uq_host_organization_member_user"),
    # CASCADE: a member row is part of its organization rather than a thing
    # that outlives it.
    sa.ForeignKeyConstraint(
        ["tenant_id", "organization_id", "unit_id"],
        ["host_organization.tenant_id", "host_organization.id", "host_organization.unit_id"],
        ondelete="CASCADE",
    ...
    sa.Index("ix_host_organization_member_organization", "tenant_id", "organization_id"),
)
