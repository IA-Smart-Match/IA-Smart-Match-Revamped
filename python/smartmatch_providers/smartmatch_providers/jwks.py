...
The RS256 check is delegated to :mod:`cryptography` — ``RSAPublicKey.verify``
with PKCS#1 v1.5 padding over SHA-256. There is no modular arithmetic, no
hand-built ASN.1 DigestInfo, and no padding comparison anywhere in this file.
An authentication path is the last place to carry a bespoke implementation of a
primitive that a reviewed library already provides, and ``cryptography`` is
hash-pinned in ``requirements/runtime.txt``. This module handles no secret
material: the private half of every key used against it exists only inside the
tests, which generate throwaway keys at run time.
"""
...
#: IdP issues; present so a fixture key too small to be a meaningful test of the
#: signature check is rejected rather than quietly accepted.
...
class RSAPublicJWK:
    ...
    def __post_init__(self) -> None:
        ...
        The last check hands the numbers to :mod:`cryptography`, the component
        that will actually use them. A key this module accepted but that
        library refused would surface later as a verification failure, which
        reads as "bad token" when it is really "bad configuration".
    ...
    def public_key(self) -> rsa.RSAPublicKey:
        """Build the :mod:`cryptography` public key these numbers describe.

        Raises:
            ProviderConfigurationError: if the library refuses the numbers.
        """
        try:
            return rsa.RSAPublicNumbers(e=self.exponent, n=self.modulus).public_key()
        except ValueError as exc:
            raise ProviderConfigurationError(
                f"JWKS key {self.kid!r} is not a usable RSA public key"
            ) from exc
...
def _rs256_signature_is_valid(key: RSAPublicJWK, signing_input: bytes, signature: bytes) -> bool:
    """Check an RSASSA-PKCS1-v1_5 SHA-256 signature.

    The check itself belongs to :mod:`cryptography`. This function exists only
    to turn its exception into a boolean so the caller's control flow stays
    readable, and deliberately contains no arithmetic of its own.
...
class StaticJWKSTokenVerifier:
    ...
    def verify(self, token: str) -> VerifiedIdentity:
        ...
        RS256``; the signature over the signing input; then ``exp``, ``nbf``,
        ``iss``, and ``aud`` against this verifier's arguments. Claims are read
        only after the signature holds, so an unsigned token's claims never
        influence anything.
