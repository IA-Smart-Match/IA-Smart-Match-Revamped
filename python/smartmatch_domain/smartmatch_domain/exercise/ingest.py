...
class ExerciseFileLayout:
    ...
    Attributes:
        ...
        class_year_column: The year. **No vocabulary** — see the module
            docstring.
...
class ParsedProfile:
    ...
    nullable column. ``hidden_true_interests`` is ADR-0025 D6's withheld list:
    carried here so the repository can store it, and named on no report.
    """
...
class IngestReport:
    ...
    vocabulary is undecided, so this module refuses to invent one and reports
    what the file actually used instead.

    ``unmapped_*`` counts are ADR-0011's "counted, never silently dropped"
    under a mapping that does not exist yet: no term is mapped to G3 here, so
    every distinct term is reported as-is and every row is kept.
    """
